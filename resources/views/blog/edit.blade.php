...
#editor-area h2 { font-size: 1.75rem; font-weight: 700; margin: 1.5rem 0 0.75rem; color: #151c27; }
#editor-area h3 { font-size: 1.35rem; font-weight: 600; margin: 1.25rem 0 0.5rem; color: #151c27; }
#editor-area p  { margin-bottom: 1rem; }
#editor-area ul { list-style: disc; padding-left: 2rem; margin-bottom: 1rem; }
#editor-area ol { list-style: decimal; padding-left: 2rem; margin-bottom: 1rem; }
...
.tbtn:hover { background: #dce2f3; color: #151c27; }
.tbtn.on { background: #b61722; color: #fff; }
.tsep { width: 1px; background: #e4beba; margin: 4px 6px; }
</style>
...
<div class="max-w-article-max mx-auto">
    ...
    <form method="POST" action="/posts/{{ $post->id }}"
        ...
        <div class="mb-8">
            ...
            @if($post->cover_image)
                <div id="existing-image" class="relative rounded-xl overflow-hidden mb-2">
                    ...
                    <p class="absolute bottom-3 left-3 text-white text-xs font-medium">
            ...
            <div id="drop-zone"
                 class="{{ $post->cover_image ? 'hidden' : '' }} border-2 border-dashed border-gray-300 rounded-xl p-10 text-center cursor-pointer hover:border-red-400 hover:bg-red-50 transition-all duration-200">
        ...
        <div class="mb-8">
            ...
            <input type="hidden" name="body" id="body-input" value="{{ old('body', $post->body) }}"/>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (bodyInput.value && bodyInput.value.trim() !== '') {
    ...
    function updateToolbar() {
        setOn('btn-bold',      document.queryCommandState('bold'));
        setOn('btn-italic',    document.queryCommandState('italic'));
        setOn('btn-underline', document.queryCommandState('underline'));
        setOn('btn-h2',        isTag('H2'));
        setOn('btn-h3',        isTag('H3'));
        setOn('btn-bullet',    document.queryCommandState('insertUnorderedList'));
        setOn('btn-ordered',   document.queryCommandState('insertOrderedList'));
        setOn('btn-quote',     isTag('BLOCKQUOTE'));
    }
    ...
    fileInput.addEventListener('change', function () {
        ...
        if (!file) return;
        if (file.size > 2097152) { alert('Max 2MB'); this.value = ''; return; }
        const reader = new FileReader();
        reader.onload = e => {
            previewImg.src           = e.target.result;
            previewBox.style.display = 'block';
            if (dropZone) dropZone.style.display = 'none';
            const ex = document.getElementById('existing-image');
    ...
    const removeBtn = document.getElementById('remove-existing-btn');
    if (removeBtn) {
    ...
    if (dropZone) {
        dropZone.addEventListener('dragover',  e => { e.preventDefault(); dropZone.style.borderColor = '#b61722'; });
